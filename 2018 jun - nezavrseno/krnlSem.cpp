...
KernelSem::KernelSem (int init) : value(init), threadsInCriticalSection(0),
	waitBlocked(new QueuePCB()), timerBlocked(new QueuePCB()) {}
...
int KernelSem::waitFriend(int signaledByFriend){
	lock();
	if(signaledByFriend){
		if(threadsInCriticalSection == System::running->friendsInCriticalSecction){
	}else{
		value--;
		if(value<0 || (threadsInCriticalSection > System::running->friendsInCriticalSecction)){
